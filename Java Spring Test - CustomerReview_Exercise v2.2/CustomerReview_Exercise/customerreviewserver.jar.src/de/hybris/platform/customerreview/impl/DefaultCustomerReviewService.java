...
/*    */ {
	...
	/*    */ {
		/* 54 */ CustomerReview review = CustomerReviewManager.getInstance().createCustomerReview(rating, headline,
				comment, /* 55 */ (User) getModelService().getSource(user),
	...
	/*    */ {
		/* 70 */ List<CustomerReview> reviews = CustomerReviewManager.getInstance()
				.getAllReviews(/* 71 */ (Product) getModelService().getSource(product));
	...
	/*    */ {
		/* 78 */ return CustomerReviewManager.getInstance()
				.getAverageRating((Product) getModelService().getSource(product));
		/*    */ }
	...
	/*    */ {
		/* 84 */ return CustomerReviewManager.getInstance()
				.getNumberOfReviews((Product) getModelService().getSource(product));
		/*    */ }
	 //Method to get the total no of reviews based on given rating range
			public Integer getNumberOfReviewsPerRange(ProductModel product, int beginRange, int endRange) {
					return CustomerReviewManager.getInstance()
					.getNumberOfReviewsPerRange((Product) getModelService().getSource(product),beginRange,endRange  );
	}
	...
	/*    */
	/*    */
	/*    */ public List<CustomerReviewModel> getReviewsForProductAndLanguage(ProductModel product,
